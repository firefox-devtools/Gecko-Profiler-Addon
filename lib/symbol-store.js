...
function getSymbolsFromExistingDumpInObjDir(path, debugName, breakpadId) {
  return Promise.resolve().then(() => {
    ...
    if (debugSymbolStore()) {
      dump(`attempting to read sym file for library ${debugName} ${breakpadId} at ${symFilePath}\n`);
    }
...
function getSymbolDumpFromSymbolServer(debugName, breakpadId) {
  const url = urlForSymFile(debugName, breakpadId);
  if (debugSymbolStore()) {
    dump(`fetching symbol dump for library ${debugName} ${breakpadId} from ${url}\n`);
  }
 ...
 * @param  string debugName    The library's debugName.
 ...
 */
async function getSymbols(debugName, breakpadId, path, platform, arch) {
  let cachedInfo = getCachedInfo(debugName, breakpadId);
  if (cachedInfo) {
  ...
  try {
    ...
    const symbolDump = await logPromise(
      `reading existing symbol dump for library ${debugName} ${breakpadId} from the objdir`,
      getSymbolsFromExistingDumpInObjDir(path, debugName, breakpadId)
    );
    return await logPromise(
      `parsing symbol file for library ${debugName} ${breakpadId} obtained from existing symbol dump`,
      parseSym(symbolDump)
    );
  } catch (error) {
    try {
      ...
      const symbolDump = await logPromise(
        `requesting symbols for library ${debugName} ${breakpadId} from the symbol server`,
        getSymbolDumpFromSymbolServer(debugName, breakpadId)
      );
      return await logPromise(
        `parsing symbol file for library ${debugName} ${breakpadId} obtained from the symbol server`,
        parseSym(symbolDump)
      );
    } catch (error) {
      if (!haveAbsolutePath) {
...
function getCachedInfo(debugName, breakpadId) {
  return _cache.get(urlForSymFile(debugName, breakpadId));
}

function prime(libs, profilerPlatform) {
  let { platform, arch } = profilerPlatform;
  libs.forEach(lib => {
    const { debugName, breakpadId, path } = lib;
    _cache.set(urlForSymFile(debugName, breakpadId), {debugName, breakpadId, path, platform, arch});
  });
}
