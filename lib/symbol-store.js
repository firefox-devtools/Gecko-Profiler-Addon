...
Cu.importGlobalProperties(['fetch', 'Blob', 'TextDecoder', 'FileReader']);
...
function getSymbols(pdbName, breakpadId, path, platform, arch) {
  ...
  return getSymFile(pdbName, breakpadId, path, platform, arch).then(symbolDump => {
    if (symbolDump instanceof Blob) {
      const reader = new FileReader();
      return reader.readAsArrayBuffer(symbolDump);
    }
    return symbolDump;
  }).then(symbolDump => {
    if (debugSymbolStore()) {
      dump(`received symbol dump for ${pdbName} ${breakpadId}\n`);
      const text = (new TextDecoder()).decode(symbolDump);
      dump(text);
    }
    return parseSym(symbolDump).then(([addresses, symindex, symbuffer]) => {
      if (debugSymbolStore()) {
        dump(`parsed symbol dump for ${pdbName} ${breakpadId}, here's all the symbols:\n`);
        const text = (new TextDecoder()).decode(symbuffer);
        dump(text);
        dump('\n');
      }
      return [addresses, symindex, symbuffer];
    });
  });
}
