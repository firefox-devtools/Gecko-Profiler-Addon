...
function getSharedLibraryInformation() {
  ...
  if (typeof sli === 'string') {
    // If sli is a string, then we're in a pre-bug 1329111 build and need to
    // massage the data a little so that it has the shape that we need.
    ...
    return Promise.resolve(json.map(lib => {
      ...
      return Object.assign({}, lib, {pdbName, breakpadId, path});
    }));
  }

  // If sli is not a string, then we assume that bug 1329111 has landed and that
  // sli has exactly the shape that we want.
  return Promise.resolve(sli);
}
